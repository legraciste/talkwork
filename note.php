<html>
    ...
    <body>
        <h1>NOTES</h1>
        <table>
  <tr><th>Nom</th><th>Prenom</th><th>Note</th></tr>
  <?php
            include_once 'dbConnection.php';
            $req=mysqli_query($con,"SELECT utilisateurs.nom,utilisateurs.prenom,note.note FROM utilisateurs INNER JOIN note ON utilisateurs.id=note.id_note");
            $data=$req->fetch_all(MYSQLI_ASSOC);
            foreach($data as $row){
        ?>
  <tr><td><?=$row['nom']?></td><td><?=$row['prenom']?></td><td><?=$row['note']?></td></tr>
  <?php } ?>
       </table>
    </body>
</html>